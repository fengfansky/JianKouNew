...
public class RoomManager extends Observable {
     ...
     * 获取Room内所有用户
     */
    public void FetchAllUserInRoom(final String roomId, final GetAllUserInRoomCallback callback) {

        users = new ArrayList<User>();
        getAllUserInRoom(roomId, new GetAllUserInRoomCallback() {
            ...
            @Override
            public void onFailure(String errorMsg) {
                Log.d("FetchAllUserInRoom", "FetchAllUserInRoom Failture... ");
            }
        });
    ...
    private void getAllUserInRoom(final String roomId, final GetAllUserInRoomCallback callback) {
        new Thread(new Runnable() {
            @Override
            public void run() {

                Map<String, Object> params = new HashMap<String, Object>();
                params.put("circle_ids", roomId);
                params.put("need_user_detail", true);

                try {
                    anSocial.sendRequest("circles/get.json", AnSocialMethod.GET, params, new IAnSocialCallback() {
                        @Override
                        public void onFailure(final JSONObject arg0) {
                            try {
                                String message = arg0.getJSONObject("meta").getString("message");
                        ...
                        public void onSuccess(JSONObject arg0) {
                            try {
                                ...
                                JSONArray circlesArray = arg0.getJSONObject("response").getJSONArray("circles");
                                JSONObject circleObject = circlesArray.getJSONObject(0);
                                JSONArray userArray = circleObject.getJSONArray("users");
                                ...
                                Log.d("ddd", "users size: " + users.size());
                                handler.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        if (callback != null) {
                        ...
                        public void onSuccess(JSONObject arg0) {
                            ...
                            try {
                                ...
                                for (int i = 0; i < likeArray.length(); i++) {
                                    ...
                                    like.parseJSON(likeArray.getJSONObject(i), UserManager.getInstance(ct)
    ...
    public interface GetAllUserInRoomCallback {
        public void onFailure(String errorMsg);
