...
public class UserManager extends Observable {
            ...
            anSocial.sendRequest("friends/requests/reject.json", AnSocialMethod.POST, params, new IAnSocialCallback() {
                ...
                public void onSuccess(final JSONObject arg0) {
                    ...
                    try {
                        ...
                        c_data.put(Constant.FRIEND_REQUEST_ID, request.friendRequestId);
                        c_data.put(Constant.FRIEND_REQUEST_KEY_TYPE, Constant.FRIEND_REQUEST_TYPE_REJECT);
                        IMManager
                                .getInstance(ct)
                                .getAnIM()
    ...
    public void removeFriendLocal(String targetClientId) {
//        if (currentUser.isFriend(targetClientId)) {
            currentUser.removeFriend(currentUser.clientId, targetClientId);
//            setChanged();
//            notifyObservers(UpdateType.Friend);
//        }
    }
    ...
    public void getUserByUserIdIfExist(String userId, FetchSingleUserCallback callback) {
        User user = new Select().from(User.class).where("userId = ?", userId).executeSingle();
        if (null != user) {
            callback.onFinish(user);
        } else {
            fetchSIngleUserDataByUserId(userId, callback);
        }
    }
