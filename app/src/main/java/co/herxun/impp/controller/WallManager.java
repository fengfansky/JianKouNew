                        ...
                        public void onSuccess(JSONObject arg0) {
                            ...
                            try {
                                ...
                                for (int i = 0; i < postArray.length(); i++) {
                                    ...
                                    if (postJson.has("like")) {
                                        post.deleteAllLikes();
                                        JSONObject likeJson = postJson.getJSONObject("like");
                                        Like like = new Like();
                                        like.post = post.getFromTable();
                                        like.parseJSON(likeJson, UserManager.getInstance(ct).getCurrentUser().userId);
                                        if (like.owner == null) {
                                            like.owner = UserManager.getInstance(ct).getCurrentUser().getFromTable();
                                        }
