...
public class User extends Model implements Serializable {
    ...
    public void removeFriend(String clientId, String targetClientId) {
        Friend friend = new Select().from(Friend.class)
                .where("userClientId = \"" + clientId + "\" and targetClientId = \"" + targetClientId + "\"")
                .executeSingle();
        if (friend != null) {
            friend.delete();
        }
    ...
    public List<User> friends() {
        ...
        List<Friend> fs = new Select().from(Friend.class).where("userClientId = \"" + clientId + "\" and isMutual = 1")
                .execute();
        for (Friend f : fs) {
    ...
    public boolean isFriend(String targetClientId) {
        List<Friend> fs = new Select()
                .from(Friend.class)
                .where("userClientId = \"" + clientId + "\" and targetClientId = \"" + targetClientId
                        + "\" and isMutual = 1").execute();
        return fs.size() > 0;
    }
    ...
    public boolean same() {
        User userExisit = new Select().from(User.class).where("clientId = ? ", clientId).executeSingle();
        if ((userExisit != null) && userExisit.userName != null && userExisit.userId != null
                && (userExisit.userId.equals(userId)) && (userExisit.userName.equals(userName))) {
