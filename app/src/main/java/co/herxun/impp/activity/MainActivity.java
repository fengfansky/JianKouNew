...
//            frag.onViewShown();
    ...
    private void checkBundle() {
        if (getIntent().hasExtra(Constant.INTENT_EXTRA_KEY_PAYLOAD)) {
            String payload = getIntent().getStringExtra(Constant.INTENT_EXTRA_KEY_PAYLOAD);
            String alert = null;
            ...
            try {
                ...
                if (json.has("topic_id")) {
                    ...
                    chat = IMManager.getInstance(this).addChat(topic);
                	 ...
                	 if (json.has("file_type") && json.getString("file_type").equals(Constant.FRIEND_REQUEST_TYPE_SEND)) {
                         // should go to friend page
                     } else {
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (mChatListFragment.isAdded()) {
            getSupportFragmentManager().putFragment(outState, ChatListFragment.class.getName(), mChatListFragment);
        }
        if (mFriendListFragment.isAdded()) {
            getSupportFragmentManager().putFragment(outState, FriendListFragment.class.getName(), mFriendListFragment);
        }
        if (mExploreFragment.isAdded()) {
            getSupportFragmentManager().putFragment(outState, ExploreFragment.class.getName(), mExploreFragment);
        }
        if (mSettingFragment.isAdded()) {
            getSupportFragmentManager().putFragment(outState, SettingFragment.class.getName(), mSettingFragment);
        }
    }

    private void initView(Bundle savedInstanceState) {
        if (savedInstanceState != null) {
            mChatListFragment = (ChatListFragment) getSupportFragmentManager().getFragment(savedInstanceState,
                    ChatListFragment.class.getName());
            mFriendListFragment = (FriendListFragment) getSupportFragmentManager().getFragment(savedInstanceState,
                    FriendListFragment.class.getName());
            mExploreFragment = (ExploreFragment) getSupportFragmentManager().getFragment(savedInstanceState,
                    ExploreFragment.class.getName());
            mSettingFragment = (SettingFragment) getSupportFragmentManager().getFragment(savedInstanceState,
    ...
    private void onChatListFragmentShown(final ChatListFragment frag) {
        ...
        mAppbar.getMenuItemView().setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                LayoutParams rlpLogo = (LayoutParams) mAppbar.getLogoView().getLayoutParams();
